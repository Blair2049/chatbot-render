...
async def openai_complete_if_cache(
    ...
    **kwargs,
    ...
    if not api_key:
        ...
        if not api_key:
            raise ValueError("API Key is not set. Please set OPENAI_API_KEY in .env or pass it as a parameter.")

    if not model:
        model = os.getenv("OPENAI_MODEL", "gpt-4o-mini")

    openai_async_client = (
        AsyncOpenAI(api_key=api_key) if base_url is None else AsyncOpenAI(base_url=base_url)
    )
